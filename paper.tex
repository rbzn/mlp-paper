...
In the \emph{Mathematical Language Processing} project, we discover definitions %we did not discover any relations
% a relation would be something like "let <subj> be a <object>", or "we call <subj> <obj>", or "<subject> is <object>" or just "<subj> <obj>"
 for identifiers in formulae from the surrounding text by statistical methods.
 %with a machine learning approach for classification. This was also not done in this paper
%We extract the identifiers from the formulae and track co-occurrences of those in the surrounding text.
